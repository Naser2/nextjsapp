...
export function PlayButton({ player, size = 'large', className }) {
  console.log('PLAYER-BUTTON', player)
  console.log('PLAYER-BUTTON-TOGGLE', player.toggle)
  console.log('PLAYER-BUTTON-IS-PLAYING', player.isPlaying)
  return (
    <button
      id="play-btn-Glodal"
      type="button"
      className={clsx(
        ...
        // 'group relative flex flex-shrink-0 items-center justify-center rounded-full bg-slate-700 hover:bg-slate-900 focus:outline-none focus:ring-slate-700',
        {
          ...
          medium: 'h-14 w-14 focus:ring-2 focus:ring-offset-2  md:h-16 md:w-16',
          small: 'h-10 w-10  focus:ring-2 focus:ring-offset-2 md:h-12 md:w-12',
        }[size]
      )}
...
export function CloseButton({ togglePartial, size, className, color, props }) {
  console.log('ClOSESSS-BUTTON', props)
  ...
  return (
    <button
      ...
      onClick={() => togglePartial(false)}
    >
      <div className="absolute -inset-3" />
      <ArrowLeftIcon className={className} size={size} color={color} />
    </button>
  )
}
