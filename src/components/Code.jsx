...
function CopyButton({ code }) {
  ...
  return (
    <button
      type="button"
      className={clsx(
        'group/button text-2xs absolute top-3.5 right-4 overflow-hidden rounded-full py-1 pl-2 pr-3 font-medium opacity-0 backdrop-blur transition focus:opacity-100 group-hover:opacity-100',
        copied
...
function CodeGroupHeader({ title, children, selectedIndex }) {
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-b border-zinc-700 bg-zinc-800 px-4 dark:border-zinc-800 dark:bg-transparent">
      ...
      {hasTabs && (
        <Tab.List className="-mb-px flex gap-4 text-xs font-extrabold">
          {Children.map(children, (child, childIndex) => (
            <Tab
              className={clsx(
                'border-b py-3 transition focus:[&:not(:focus-visible)]:outline-none',
                childIndex === selectedIndex
                  ? 'code-li-property-value border-orange-500 font-bold'
